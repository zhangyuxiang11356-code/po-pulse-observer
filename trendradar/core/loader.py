...
from trendradar.sources import build_source_catalog, group_source_catalog
...
def _load_app_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "SHOW_VERSION_UPDATE": app_config.get("show_version_update", False),
...
def _load_display_config(config_data: Dict) -> Dict:
    ...
    valid_regions = {"hotlist", "rss", "new_items", "standalone", "ai_analysis", "social_media"}
    ...
    return {
        ...
        "REGIONS": {
            ...
            "SOCIAL_MEDIA": regions.get("social_media", True),
        },
        # 独立展示区配置
        "STANDALONE": {
...
def _load_social_media_config(config_data: Dict) -> Dict:
    """加载社交媒体配置。"""
    social = config_data.get("social_media", {})
    advanced = config_data.get("advanced", {})
    advanced_crawler = advanced.get("crawler", {})
    social_proxy_url = social.get("proxy_url", "") or advanced_crawler.get("default_proxy", "")

    return {
        "ENABLED": social.get("enabled", False),
        "REQUEST_INTERVAL": social.get("request_interval", 2000),
        "TIMEOUT": social.get("timeout", 15),
        "USE_PROXY": social.get("use_proxy", False),
        "PROXY_URL": social_proxy_url,
        "MAX_ITEMS_PER_SOURCE": social.get("max_items_per_source", 10),
        "MAX_AGE_DAYS": social.get("max_age_days", 0),
...
def _load_ai_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "FALLBACK_API_KEY": _get_env_str("AI_FALLBACK_API_KEY") or "",
        "FALLBACK_API_BASE": _get_env_str("AI_FALLBACK_API_BASE") or "",
        "FALLBACK_MODEL": _get_env_str("AI_FALLBACK_MODEL") or "",
        ...
        "FORCE_STREAM": _get_env_bool("AI_FORCE_STREAM") if _get_env_bool("AI_FORCE_STREAM") is not None else ai_config.get("force_stream", False),
        "AUTO_STREAM_FALLBACK": _get_env_bool("AI_AUTO_STREAM_FALLBACK") if _get_env_bool("AI_AUTO_STREAM_FALLBACK") is not None else ai_config.get("auto_stream_fallback", True),
        "USE_RESPONSES_API": _get_env_bool("AI_USE_RESPONSES_API") if _get_env_bool("AI_USE_RESPONSES_API") is not None else ai_config.get("use_responses_api", False),
        "MIN_REQUEST_INTERVAL_SECONDS": float(_get_env_str("AI_MIN_REQUEST_INTERVAL_SECONDS") or ai_config.get("min_request_interval_seconds", 0) or 0),
...
def _load_ai_analysis_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "INCLUDE_SOCIAL": ai_config.get("include_social", True),
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    config["SOURCE_CATALOG"] = build_source_catalog(config_data)
    config["SOURCE_CATALOG_GROUPS"] = group_source_catalog(config["SOURCE_CATALOG"])

    # RSS 配置
    config["RSS"] = _load_rss_config(config_data)
    config["SOCIAL_MEDIA"] = _load_social_media_config(config_data)
