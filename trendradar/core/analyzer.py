...
def count_rss_frequency(
    ...
    for item in rss_items:
        ...
        match_text = item.get("match_text") or " ".join(
            part for part in [title, item.get("summary", ""), item.get("article_text", "")] if part
        )
        url = item.get("url", "")
        ...
        # 使用统一的匹配逻辑
        if not matches_word_groups(match_text, word_groups, filter_words, global_filters):
            continue
