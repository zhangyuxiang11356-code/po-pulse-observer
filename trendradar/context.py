...
class AppContext:
    ...
    @property
    def social_media_config(self) -> Dict:
        return self.config.get("SOCIAL_MEDIA", {})

    @property
    def social_media_enabled(self) -> bool:
        return self.social_media_config.get("ENABLED", False)

    @property
    def social_media_sources(self) -> List[Dict]:
        return self.social_media_config.get("SOURCES", [])

    @property
    def source_catalog_entries(self) -> List[Dict]:
        """获取统一信源目录。"""
        return self.config.get("SOURCE_CATALOG", [])

    @property
    def source_catalog_groups(self) -> Dict[str, List[Dict]]:
        """获取按页面分组后的统一信源目录。"""
        return self.config.get("SOURCE_CATALOG_GROUPS", {})
    ...
    def region_order(self) -> List[str]:
        """获取区域显示顺序"""
        default_order = ["hotlist", "rss", "new_items", "standalone", "ai_analysis", "social_media"]
    ...
    def filter_social_items(self, social_items: Optional[List[Dict]], interests_file: Optional[str] = None) -> List[Dict]:
        """社交媒体内容仅做日期过滤，不复用 AI 主体筛选。"""
        if not social_items:
            return []
        max_age_days = int(self.social_media_config.get("MAX_AGE_DAYS", 0) or 0)
        if max_age_days <= 0:
            return social_items
        ...
        for item in social_items:
            ...
            if not published_at:
                ...
                continue
            if is_within_days(str(published_at), max_age_days, timezone=self.timezone):
                filtered_items.append(item)
            else:
                dropped_count += 1

        print(
            f"[Social] 日期过滤完成：保留 {len(filtered_items)}/{len(social_items)} 条 "
            f"(近 {max_age_days} 天，过滤 {dropped_count} 条过期内容)"
        )

        if filtered_items:
            return filtered_items

        # 在线源全部失效时，仅允许退回到“同日”的宿主机缓存。
        stale_cache_items = []
        for item in social_items:
            ...
            if source in {
                ...
                stale_cache_items.append(item)

        if stale_cache_items:
            print(
                f"[Social] 新鲜内容为空，回退保留 {len(stale_cache_items)}/{len(social_items)} 条同日宿主机缓存内容"
            )
            return stale_cache_items
