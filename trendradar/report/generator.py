...
from trendradar.report.archive import generate_archive_pages
...
def generate_html_report(
    ...
    2. 在允许时复制到 output/html/latest/{mode}.html（最新报告兼容别名）
    3. 在允许时复制到 output/index.html 和根目录 index.html（站点真实首页）
    ...
    Args:
        ...
        matches_word_groups_func: 词组匹配函数
        load_frequency_words_func: 加载频率词函数
        publish_latest: 是否更新 latest/{mode}.html 兼容别名
        publish_entry_index: 是否更新 output/index.html 与根目录 index.html 为真实首页
    ...
    # 2. 在允许时复制到 html/latest/{mode}.html（最新报告兼容别名）
    if publish_latest:
    ...
    if publish_entry_index:
        ...
        # 根目录 index.html（供 GitHub Pages / 本地工作区访问）
        root_index = Path("index.html")
        with open(root_index, "w", encoding="utf-8") as f:
            f.write(html_content)

    generate_archive_pages(output_dir)
