...
def _get_override_time(timezone: str) -> Optional[datetime]:
    """
    读取环境变量覆盖时间。

    支持:
    - TRENDRADAR_NOW=2026-04-08T09:30:00+08:00
    - TRENDRADAR_NOW=2026-04-08 09:30:00
    """
    override = os.environ.get("TRENDRADAR_NOW", "").strip()
    if not override:
        return None
    ...
    try:
        ...
        print(f"[警告] TRENDRADAR_NOW 格式错误: {override}")
        return None
...
def get_configured_time(timezone: str = DEFAULT_TIMEZONE) -> datetime:
    ...
    Returns:
        带时区信息的当前时间
    """
    override_time = _get_override_time(timezone)
...
def is_same_local_date(
    iso_time: str,
    timezone: str = DEFAULT_TIMEZONE,
    reference_time: Optional[datetime] = None,
) -> bool:
    """
    判断 ISO 时间在目标时区下是否与 reference_time 同一天。
    """
    if not iso_time:
        return False
    ...
    ref_dt = reference_time.astimezone(target_tz) if reference_time else get_configured_time(timezone)
    return local_dt.date() == ref_dt.date()
