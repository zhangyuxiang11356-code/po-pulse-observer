...
CONTAINER_ENGINE = os.environ.get("CONTAINER_ENGINE", "podman").strip() or "podman"
CONTAINER_NAME = os.environ.get("CONTAINER_NAME", "trendradar").strip() or "trendradar"
COMPOSE_COMMAND = f"{CONTAINER_ENGINE} compose"


def engine_cmd(args: str) -> str:
    """Return a host-side container engine command for docs/help output."""
    return f"{CONTAINER_ENGINE} {args}".strip()
...
def show_status():
    ...
    if supercronic_is_pid1:
        ...
        print("    💡 建议操作:")
        print(f"       • 重启容器: {engine_cmd(f'restart {CONTAINER_NAME}')}")
        print(f"       • 检查容器日志: {engine_cmd(f'logs {CONTAINER_NAME}')}")

    # 显示日志检查建议
    print("  📋 运行状态检查:")
    print(f"    • 查看完整容器日志: {engine_cmd(f'logs {CONTAINER_NAME}')}")
    print(f"    • 查看实时日志: {engine_cmd(f'logs -f {CONTAINER_NAME}')}")
    print("    • 手动执行测试: python manage.py run")
    print(f"    • 重启容器服务: {engine_cmd(f'restart {CONTAINER_NAME}')}")
...
📖 使用示例:
  ...
  # 在宿主机执行
  {engine_cmd(f'exec -it {CONTAINER_NAME} python manage.py run')}
  {engine_cmd(f'exec -it {CONTAINER_NAME} python manage.py status')}
  {engine_cmd(f'exec -it {CONTAINER_NAME} python manage.py start_webserver')}
  {engine_cmd(f'logs {CONTAINER_NAME}')}
